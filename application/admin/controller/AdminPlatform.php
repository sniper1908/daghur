...
class AdminPlatform extends Controller
{
	...
	public function _initialize()
	{
		...
		// 非add/edit页面，需要用到script文件，则hasScript=1
		$this->assign('hasScript',0);
		# menu 左侧导航列表
		$this->assign('menu',$this->getNav());
		# 获取网站的配置信息
		$ShopConfigModel = new ShopConfigModel;
		$this->ShopConfig = $ShopConfigModel->getAttrValue();
	}
	...
	public function checkAdminLogin()
	{
		...
		$this->assign('sessionAdmin',$session_admin);
	}
	...
	{
		if (!$this->ShopConfig['permission_open']) {
			# 不用检查权限
			return true;
		}
	 ...
	 * @param [string] $dir_name [上传文件保存目录名]
	...
	protected function upFile($file,$fileType='image',$dir_name='image',$if_thumb=-1,$thumbSize=[300,300,Image::THUMB_CENTER],$imageSize=[0,0,Image::THUMB_CENTER])
	{
		...
		$res = [];
		if ($fileType=='image') {
			$originalUrl = $fileName.'_original.'.$fileNameArr[1];
			// 原图名称
			// $fileOriginalUrl = $dirName.'/'.$fileName.'_original.'.$fileNameArr[1];
			$fileOriginalUrl = $dirName .'/'.$originalUrl;
			// 缩略图名称
			if ($fileType=='image' && $if_thumb!=-1) {
				$fileThumbUrl = $dirName.'/'.$fileName.'_thumb.'.$fileNameArr[1];
			}
			# 移动图片，图片名称为原图名称
			$file->move($dirName,$originalUrl);
			# 原图路径
			$res['file_original_url'] = $fileOriginalUrl;
			// 生成大图
			$images = Image::open($fileOriginalUrl);
			$imageSize[0] = (isset($imageSize[0]) && $imageSize[0]) ? $imageSize[0] : $images->width();
			$imageSize[1] = (isset($imageSize[1]) && $imageSize[1]) ? $imageSize[1] : $images->height();
			// $images->thumb(300,300)->save($fileThumbUrl);
			$imageSize[2] = (isset($imageSize[2]) && $imageSize[2]) ? $imageSize[2] : Image::THUMB_CENTER;
			$images->thumb($imageSize[0],$imageSize[1],$imageSize[2])->save($fileUrl);
			# 大图路径
